...
const HighLowIndicator = ({ currentPrice, high, low }) => {
  ...
  return (
    <>
      <span className="bg-red-600 h-1.5 rounded-l-lg w-[50%]" style={{ width: `${100 - green}%` }}> &nbsp;</span>
      <span className="bg-green-600 h-1.5 rounded-r-lg w-[50%]" style={{ width: `${green}%` }}> &nbsp; </span>
    </>
  );
};
...
const CryptoDetails = () => {
  ...
  return ReactDOM.createPortal(
    <div
      className="fixed top-0 w-full h-full bg-gray-500 bg-opacity-30 first-letter:
    backdrop-blur-sm flex items-center justify-center font-sans
    "
      onClick={close}
    >
      <div
        className="w-[65%] h-[75%] bg-gray-900 bg-opacity-90 rounded-lg text-white relative font-bold"
              ...
              <div className="flex w-full mt-4 justify-between">
                <div className="flex flex-col">
                  <a
                    target={"_blank"}
                    rel="noreferrer"
                    className="text-sm bg-gray-600 text-gray-100 px-1.5 py-0.5 my-1 rounded"
